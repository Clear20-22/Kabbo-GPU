...
# Updated to point to your new YOLO11l training directory
DEFAULT_WEIGHTS = BASE_DIR / "runs" / "yolo11l_modified_all-2" / "weights" / "best.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a RoadVision DUET Kaggle-style submission for YOLO11l.")
    parser.add_argument("--weights", type=Path, default=DEFAULT_WEIGHTS)
    parser.add_argument("--test-images", type=Path, default=ROADVISION_DIR / "test" / "images")
    parser.add_argument("--output", type=Path, default=BASE_DIR / "submission_yolo11l.csv")
    
    # --- STRATEGIC ARGUMENT ADJUSTMENTS ---
    # Native 1280 resolution (divisible by 32, so no padding overhead)
    parser.add_argument("--imgsz", type=int, default=1280)
    parser.add_argument("--conf", type=float, default=0.15)  # Strict confidence to prevent "shotgun" spam
    parser.add_argument("--iou", type=float, default=0.45)   # Clears overlapping duplicate boxes
    parser.add_argument("--device", default=0)
    parser.add_argument("--max-det", type=int, default=300)
    
    # --- PHYSICAL FILTER SETTING ---
    parser.add_argument("--min-dim", type=float, default=0.001) # Prevents zero-area boxes
    ...
    print(f"📦 Loading fine-tuned model: {args.weights.parent.parent.name} -> {args.weights.name}")
    ...
    print(f"🚀 Running sequential inference against {total_frames} frames at {args.imgsz}px...")
    ...
    with args.output.open("w", newline="", encoding="utf-8") as f:
        ...
        # Process frames ONE by ONE to maintain strict VRAM control
        for idx, image_path in enumerate(test_files, 1):
            ...
            # Run inference explicitly wrapped in torch.inference_mode() for max memory efficiency
            ...
            # Explicitly delete the result and box tensors to free VRAM for the next loop iteration
            del result
            del boxes
