...
def load_csv(path: Path) -> CsvStats:
    ...
    for row in rows:
        ...
        if has_prediction_string:
            ...
            detections += 1
            class_counts[cls] += 1
            class_confidences[cls].append(1.0)
            widths.append(width)
            heights.append(height)
            areas.append(width * height)
        else:
...
def compare(stats_a: CsvStats, stats_b: CsvStats) -> None:
    ...
    if overlap:
        first_overlap = ids_a & ids_b
        print(f"\nshared IDs sample: {sorted(first_overlap)[:5]}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Compare two RoadVision DUET CSV files and print class-wise statistics.")
    parser.add_argument("csv_a", help="First CSV file name or path")
    parser.add_argument("csv_b", help="Second CSV file name or path")
    parser.add_argument("--output-json", type=Path, default=None, help="Optional path to save the comparison summary as JSON")
    args = parser.parse_args()

    path_a = resolve_csv(args.csv_a)
    path_b = resolve_csv(args.csv_b)

    stats_a = load_csv(path_a)
    stats_b = load_csv(path_b)
    ...
    if args.output_json is not None:
        summary = {
            "first": {
                "path": str(stats_a.path),
                "rows": stats_a.row_count,
                "detections": stats_a.detections,
                "empty_prediction_rows": stats_a.empty_prediction_rows,
                "malformed_prediction_rows": stats_a.malformed_prediction_rows,
                "class_counts": dict(stats_a.class_counts),
            },
            "second": {
                "path": str(stats_b.path),
                "rows": stats_b.row_count,
                "detections": stats_b.detections,
                "empty_prediction_rows": stats_b.empty_prediction_rows,
                "malformed_prediction_rows": stats_b.malformed_prediction_rows,
                "class_counts": dict(stats_b.class_counts),
            },
            "comparison": {
                "shared_image_ids": len(set(stats_a.image_ids) & set(stats_b.image_ids)),
                "only_first": len(set(stats_a.image_ids) - set(stats_b.image_ids)),
                "only_second": len(set(stats_b.image_ids) - set(stats_a.image_ids)),
                "row_delta": stats_a.row_count - stats_b.row_count,
                "detection_delta": stats_a.detections - stats_b.detections,
            },
        }
